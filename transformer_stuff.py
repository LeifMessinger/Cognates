...
class TransformerCognateModel(nn.Module):
	def __init__(self, embedder, hidden_dim=64, positional_dropout=0.2, dropout=0.0, layers=1):
		...
		self.fc = nn.Sequential(
			nn.Linear(embedding_dim * 3, 64),  # Changed from hidden_dim * 2 to embedding_dim * 2
			nn.ReLU(),
			nn.Linear(64, 1)#,
			#nn.Sigmoid()
		)
	...
	def forward(self, word_pair, word_pair_masks):
		...
		import random
		combined = torch.cat([enc1, enc2, (enc1 - enc2).abs()] if random.choice([True, False]) else [enc2, enc1, (enc1 - enc2).abs()], dim=1)  # [batch_size, embedding_dim * 2]
